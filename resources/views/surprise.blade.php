@php
    ...
    $coverImage = $profile->cover_image ? asset('storage/'.$profile->cover_image) : asset('storage/birthday/memories/lakeside-demo.png');
...
<body class="surprise-page floral-page" style="--accent: {{ $settings->primary_color ?? '#8a6940' }}; --gold: {{ $settings->secondary_color ?? '#8a6940' }}; --cover-image: url('{{ $coverImage }}')">
    ...
    <div class="site-frame" id="top">
        ...
        <main>
            ...
            <section id="timeline" class="floral-section floral-story">
                ...
                <div class="floral-timeline">
                    @forelse($memories as $memory)
                        <article class="floral-timeline-item reveal"><div class="floral-timeline-dot">✦</div><div class="floral-timeline-card"><div class="floral-timeline-image">@if($memory->cover_image)<img loading="lazy" src="{{ asset('storage/'.$memory->cover_image) }}" alt="{{ $memory->title }}">@else<div>✦</div>@endif</div><div><span class="floral-date">{{ $memory->memory_date?->format('d M Y') ?? 'a beautiful day' }}</span><h3>{{ $memory->title }}</h3><p>{{ $memory->short_description }}</p><button class="floral-text-button" type="button" data-bs-toggle="modal" data-bs-target="#memory-{{ $memory->id }}">Read this chapter ↗</button></div></div></article>
                        <div class="modal fade dark-modal" id="memory-{{ $memory->id }}" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><button type="button" class="modal-close" data-bs-dismiss="modal" aria-label="Close">×</button>@if($memory->cover_image)<img src="{{ asset('storage/'.$memory->cover_image) }}" alt="{{ $memory->title }}">@endif<div class="modal-body"><span class="memory-date">{{ $memory->memory_date?->format('d M Y') }}</span><h3>{{ $memory->title }}</h3><p>{!! nl2br(e($memory->full_description ?: $memory->short_description)) !!}</p></div></div></div></div>
                    @empty
            ...
            <section class="floral-section floral-featured"><div class="floral-heading"><p class="floral-eyebrow">03 · the highlights</p><h2>Some days feel<br><em>like keepsakes.</em></h2></div><div class="floral-featured-grid">@foreach($memories->where('is_featured', true)->take(3) as $memory)<article><div class="floral-featured-image">@if($memory->cover_image)<img loading="lazy" src="{{ asset('storage/'.$memory->cover_image) }}" alt="{{ $memory->title }}">@endif</div><span>{{ $memory->memory_date?->format('M Y') }}</span><h3>{{ $memory->title }}</h3></article>@endforeach</div></section>

            <section id="gallery" class="floral-section floral-gallery"><div class="floral-heading centered"><p class="floral-eyebrow">04 · the visual diary</p><h2>Little frames of <em>us.</em></h2><p>Proof that the best parts of life are usually the unplanned ones.</p></div><div class="floral-gallery-grid">@forelse($gallery as $photo)<button class="floral-gallery-tile" type="button" data-lightbox data-src="{{ asset('storage/'.$photo->image) }}" data-caption="{{ $photo->caption }}" data-date="{{ $photo->memory_date?->format('d M Y') }}"><img loading="lazy" src="{{ asset('storage/'.$photo->image) }}" alt="{{ $photo->title ?: 'A memory of us' }}"><span>View frame ↗</span></button>@empty<div class="floral-empty">✦<p>Your gallery is waiting for its first frame.</p></div>@endforelse</div></section>
    ...
    @if($firstSong && $settings->enable_music)<div class="floral-player music-player" data-player data-video="{{ $firstSong->youtube_video_id }}" data-playlist="{{ $firstSong->youtube_playlist_id }}"><div class="player-meta"><span class="floral-player-thumb">♫</span><span><small>now playing</small><strong data-song-title>{{ $firstSong->title }}</strong></span></div><div class="player-controls"><button type="button" data-prev aria-label="Previous">◀</button><button class="play-button" type="button" data-play aria-label="Play">▶</button><button type="button" data-next aria-label="Next">▶</button></div><div class="player-progress"><span></span></div><iframe data-youtube title="Music player" allow="autoplay; encrypted-media" src="about:blank"></iframe></div>@endif
